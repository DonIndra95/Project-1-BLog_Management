...
const authorValidations = async function (req, res, next) {
  try {
    ...
    if (typeof data.fname !== "string")
      return res
        .status(400)
        .send({ status: false, msg: " Please enter first name as a String" });

    let validfname = /^\w[a-zA-Z.]*$/;

    data.fname = data.fname.trim();

    if (!validfname.test(data.fname))
      return res
        .status(400)
        .send({
          status: false,
          msg: "The first name may contain only letters",
        });
    ...
    if (typeof data.lname !== "string")
      return res
        .status(400)
        .send({ status: false, msg: "Please enter last name as a String" });

    let validlname = /^\w[a-zA-Z.]*$/;

    data.lname = data.lname.trim();

    if (!validlname.test(data.lname))
      return res
        .status(400)
        .send({ status: false, msg: "The last name may contain only letters" });
    ...
    if (!titles.includes(data.title))
      return res
        ...
        .send({
          status: false,
          msg: "Please enter title as Mr, Mrs or Miss only",
        });
    ...
    if (typeof data.password !== "string")
      return res
        .status(400)
        .send({ status: false, msg: " Please enter password as a String" });
    ...
    if (!validator.isEmail(email))
      return res
        .status(400)
        .send({ status: false, msg: "Entered email is invalid" });
